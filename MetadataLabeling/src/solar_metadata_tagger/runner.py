...
from .speed import GnssSpeedProvider, SpeedProvider
...
class CaptureService:
    def __init__(
        ...
        self.fix_store = FixHistoryStore(
        ...
        if (
            speed_provider is None
            and config.capture.trigger_mode
            == "distance"
        ):
            speed_provider = GnssSpeedProvider(
                self.fix_store
            )

        self.speed_provider = speed_provider

        self.triggers = TriggerProvider(
            config.capture,
        ...
        self.health = HealthReporter(
            ...
            interval_s=(
                config.capture.health_interval_s
            ),
        )
        ...
        try:
            ...
            for trigger in self.triggers:
                ...
                try:
                    ...
                    self.stats.record_tagging_failure(
                        exc.as_dict()
                    )
    ...
    def _recover_camera(self) -> None:
        """
        Best-effort camera reconnect after a transient
        GigE or USB failure.
        """
        ...
        try:
            ...
            LOGGER.exception(
                "Camera close failed during recovery",
                extra={
                    "event": (
                        "camera_recovery_close_failed"
                    )
                },
            )
