...
class MetadataTaggingService:
    ...
    def __init__(
        ...
        self.config = config
        self.fix_store = fix_store or FixHistoryStore(
            config.gnss.history_size
        )

        if layout_resolver is not None:
            self.layout = layout_resolver
        elif config.layout_file is not None:
            self.layout = LayoutResolver.from_geojson(
                config.layout_file
            )
        else:
            self.layout = None
        ...
        *,
        ...
        if not source.is_file():
            raise MetadataTaggerError(
                "IMAGE_NOT_FOUND",
                "Source image does not exist or is not a file.",
                path=str(source),
            )

        captured = captured_at_utc or datetime.now(
            timezone.utc
        )
        ...
        if captured.tzinfo is None:
            raise MetadataTaggerError(
                "TIMESTAMP_INVALID",
                "captured_at_utc must be timezone-aware.",
            )
        ...
        if (
            captured.year < 2000
            or captured.year > 2100
        ):
            raise MetadataTaggerError(
                "TIMESTAMP_INVALID",
                "Capture timestamp is outside the "
        ...
        storage_before = ensure_free_space(
            ...
            emergency_free_gb=(
                self.config.storage.emergency_free_gb
            ),
        )

        image_info: ImageInfo | None = None
        image_validation_error: (
            MetadataTaggerError | None
        ) = None
        ...
        if image_validation_error:
            warnings.append(
                "image_invalid:"
                f"{image_validation_error.code}"
            )

        fix = (
            manual_fix
            or self.fix_store.select_for_capture(
                captured,
                max_age_s=(
                    self.config
                    .gnss
        ...
        manifest = (
            self.config.storage.root
            / "manifests"
            / f"{self.config.mission_id}.jsonl"
        )
        ...
        try:
            ...
            payload: dict[str, Any] = {
                ...
                "image": {
                    ...
                    "relative_path": str(
                        destination_image.relative_to(
                            self.config.storage.root
                        )
                    ),
                    "media_type": _media_type(
                        destination_image.suffix
                    ),
                    "byte_size": (
                        destination_image
                        .stat()
            ...
            try:
                ...
                LOGGER.exception(
                    "Unable to write recovery record",
                    extra={
                        "event": (
        ...
        if (
            not self.config.storage.preserve_source
            and _is_inside(
                source,
                self.config
                .storage
    ...
    def _location_payload(
        ...
        if (
            ...
            .gnss
            .require_fix_quality
            and fix.fix_quality is not None
    ...
    def _assignment(
        ...
        # Retain optional manual row/panel
        ...
        if (
            ...
            if (
                not normalized_row
                or not normalized_panel
            ):
                warnings.append(
                    "optional_manual_"
                    "row_panel_incomplete"
                )
        ...
        if (
            fix is None
            or not fix.coordinates_valid
        ):
            warnings.append(
                "optional_row_panel_"
                "unresolved_no_coordinates"
            )
        ...
        assignment = self.layout.resolve(
            fix.latitude,
            fix.longitude,
        )

        if (
            assignment.row is None
            or assignment.panel is None
        ):
            warnings.append(
                "optional_row_panel_"
                "no_layout_match"
...
def _safe_camera_metadata(
    metadata: dict[str, Any],
) -> dict[str, Any]:
    # Keep JSON-compatible values and
    # prevent replacement of system metadata.
    forbidden = {
        "hardware",
        "software",
        "robot_id",
